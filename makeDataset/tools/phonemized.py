...
phonemized = phonemize(transcriptions, language="en-us", backend='espeak', preserve_punctuation=True, with_stress=True)  # Phonemize all text in one go to avoid triggering the memory protections error
...
# Sort
phonemized_lines.sort(key=lambda x: int(x[0].rsplit('_', 1)[1].split('.')[0]))
